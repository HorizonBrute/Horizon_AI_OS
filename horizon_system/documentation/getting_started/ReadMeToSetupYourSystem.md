...
4.2 The `active_env.*` file is generated by the bootstrap script (Section 5, which runs `aios_switch.py init`) or manually via `python "$HORIZON_SYSTEM/sbin/aios_switch.py" init`. The `Test-Path` / `-f` guard makes the line a harmless no-op until then.
...
- Initialize the AIOS registry + indirection layer (`aios_switch.py init`) and offer to copy `settings.json` from the template, pointed at the `aios-exec` wrapper (Step 6.2)
...
```bash
python3 "$HORIZON_SYSTEM/sbin/aios_switch.py" init           # generates ~/.horizon/bin/aios-exec.sh

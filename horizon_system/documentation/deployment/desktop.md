...
On a **single-user desktop you are the owner/admin** (Tier 1): you own `$HORIZON_ROOT` and already have full control of everything — including read/write on `brains/` — **by ownership**. So choose **Server** (enroll no humans) and do **not** enroll yourself. `horizon_humans` is for *additional non-admin operators*, not the machine owner; enrolling your own account would subject you to the group's Read-Only-on-`horizon_system` boundary — the group DENY overrides even the owner/Administrators grant, so you would lock yourself out of your own install. Use **Workstation** only to grant AIOS access to *other* humans who should not be admins (they get the tree but stay read-only on `horizon_system` and isolated from each other).

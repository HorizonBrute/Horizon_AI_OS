...
brains/<name>/
  .claude/
    ...
    skills -> $HORIZON_SYSTEM/skills_bin/   # junction; brain sees group-readable skills only
  .aios_provision.json  # provisioning manifest; do not delete
  memory/               # the brain's memory system — your design, your choice
  projects -> ...        # optional: symlinks/junctions to project folders
...
The harness must run as the brain OS user — not as the owner — to enforce the isolation boundary. The harness reads configuration from `~/.claude/`, and the brain's `~/.claude/` is junctioned to its workspace, so the harness automatically gets the brain's context when launched as the brain.
...
This checks env vars, the skills junction, git hooks, the AIOS registry, and the Deny ACLs on `sbin`, `skills_sbin`, and `logs`.
...
1.2 The bootstrap outputs (junctions, wrappers, `active_env.*` — generated by `bootstrap.{ps1,sh}`)

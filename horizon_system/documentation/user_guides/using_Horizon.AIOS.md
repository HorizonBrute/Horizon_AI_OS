...
To re-apply the same ACL model onboarding installs (without changing enrollment), run `/harden`.
...
1.1 Package the clone + bootstrap as a deployment script. Intune Win32 app wraps `git clone <org-fork-url> C:\devroot && powershell -File C:\devroot\horizon_system\sbin\bootstrap.ps1 --profile workstation --humans <operator-name-or-SID>`. A non-interactive deploy must declare the profile (`--profile server|workstation [--humans ...]`) or pass `--yes`, or the onboarding prompt will hang.
1.2 Onboarding always creates the AIOS OS groups and applies the ACL model — there is no separate hardening step. It runs as the logged-in user for the Claude Code wiring steps, and as SYSTEM for the group/ACL steps (the bootstrap detects this and handles the split automatically). On managed workstations, enroll the assigned operator into `horizon_humans` via `--humans`; on kiosks or servers use `--profile server` so no human is enrolled.
...
1.3 The ACL assignments on the filesystem (applied by onboarding/`bootstrap` and `horizon_aios_create_brain.py`; re-applicable with `horizon_aios_harden.py`), including the `horizon_humans` group and its enrolled members recorded in `.horizon_aios_deployment.json`

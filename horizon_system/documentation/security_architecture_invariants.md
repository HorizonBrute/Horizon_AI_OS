...
| `$HORIZON_PROJECTS/` | None by default (per-project grant). The parent is human create-traverse only and each human's entries are born owner-only — see the four-area self-service isolation convention below. |
| `$HORIZON_ROOT/` | None |
| `$HORIZON_SYSTEM/` (everything else: `ai_os_etc`, `templates`, `harness_configs`, `documentation`, …) | No write anywhere (read permitted) |
| `brains/<brain-name>/` | That brain's account: full. Other brains: none (isolation via ownership + per-brain group). Humans: read/write (see `horizon_humans` row). |
...
**This table covers brain account permissions only.** It does not describe what flesh-and-blood operator accounts can access. Human operators are not in the `brains` group and are not subject to the *brains* Deny ACEs above — they are owners, Administrators, or members of the AIOS-managed `horizon_humans` group. The one deliberate restriction that *does* apply to `horizon_humans` is the per-user self-service isolation carve-out on the four human areas (`projects/`, `handoffs/`, `objectives/`, `usrbin/`) below (each human reaches only their own entries; a per-area `shared/` drop-zone is the sole cross-user exception); on `brains/` humans are near-admins with full read/write.
...
|---|---|
| `$HORIZON_ROOT/` — user space *outside* `horizon_system/` and *outside* the four self-service areas (scope-local `local.*` overrides, root working files, …) | **Full control** |
| The four self-service areas — `$HORIZON_PROJECTS/`, `handoffs/`, `objectives/`, `$HORIZON_USRBIN/` (per-user isolation) | **Create-traverse on each parent** (`-wx`: create + enter, **not read/list**) + **sticky** + **NOT setgid** + an **isolating default ACL** (`default:group:horizon_humans:---`, `default:other::---`), so each human self-creates entries owned by themselves and their own private group — invisible to and undeletable by peers, with **no admin pre-create step**. Each area also has one group-shared `shared/` subdir (mode 3770, group-owned by `horizon_humans`) as the explicit cross-user/utility handoff drop-zone. **Invariant:** entries must be *created in-place* under the parent (never moved in), so the parent's default ACL is inherited. |
...
| `$HORIZON_ROOT/brains/` and `brains/<name>/` | **Read/Write** (humans are near-admins who modify brains/apps — parent `rwx` + inheritable default). Brain-to-brain isolation is unaffected: it rides on folder ownership + each brain's private group, not on the shared `brains` group, so a brain still reaches only its own workspace. |
...
An **empty** `horizon_humans` grants nobody, so on a server profile the model reduces to owner + SYSTEM + Administrators writing. In a single-user install the owner is also an `Administrators` member (and on Unix, root bypasses ACLs), so they can always write anywhere by elevating; day-to-day the owner reads and writes `brains/` and has full access to their own folder under `$HORIZON_PROJECTS/`. See §1 (User Ownership Model) for the group's lifecycle (onboarding creates it; profile gates enrollment; Administrator manages membership afterward).
...
**Config-driven posture (`file_acl_hardening.toml`).** The *what* of the model above — which principal gets which rights on which path — is declared as abstract rules in `$HORIZON_ETC/file_acl_hardening.toml`, which `horizon_aios_harden.py` translates to the native mechanism per OS (`setfacl` / `chmod +a` / `icacls`) and `horizon_aios_doctor.py` verifies from the same loader (`horizon_aios_acl_posture.py`), so enforcer and verifier can never disagree. **To customise the stance, put *only your overrides* in `$HORIZON_ETC/file_acl_hardening.local.toml`** — gitignored, deep-merged over the default (local wins, keyed by rule/group `name`; `disabled = true` drops a rule). If the default file is ever missing or corrupt the loader FAILs SECURE to an embedded copy of the shipped posture, so hardening never silently no-ops. **Caution — do not edit the shipped `file_acl_hardening.toml` directly.** It is a framework-tracked path, so the nightly sync **hard-restore overwrites it on every run** (see `$HORIZON_DOCS/system/distribution_and_updates.md`); keeping an in-place edit would mean excluding that single path from the restore lane yourself, which is **not recommended and deliberately not made easy** — overriding the security posture should be an explicit, isolated, gitignored act (the `.local.toml`), never a silent divergence inside a synced framework file. If you truly must edit the tracked file, you own the risk of the two drifting apart on the next update.

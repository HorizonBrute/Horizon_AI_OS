...
| `$HORIZON_PROJECTS/` | None by default (per-project grant). The parent is human-traverse-only and each `projects/<user>` is owner-only — see the `horizon_humans` per-user isolation convention below. |
| `$HORIZON_ROOT/` | None |
...
|---|---|
| `$HORIZON_ROOT/` — user space *outside* `horizon_system/` and *outside* `projects/` (`objectives/`, `handoffs/`, scope-local `local.*` overrides, …) | **Full control** |
| `$HORIZON_PROJECTS/` (per-user isolation) | **Traverse-only on the parent** (`--x`, not read) + **nothing on the child folders**. Each `projects/<user>` is owner-only (`0o700`, owned by that human); siblings are opaque. **New user folders are born isolated** by the parent's default ACL (`default:group:horizon_humans:---`), so adding a human needs no per-user ACL step. **Invariant:** a user folder must be *created in-place* under the parent (never moved in), so the parent's default ACL is inherited. |

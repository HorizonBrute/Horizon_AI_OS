...
|---|---|---|---|---|---|---|
| Claude Code (desktop app) | ≥1.0 | Windows 11 | Desktop | **Verified (harness); ACL isolation pending** | Verified: skills via directory junction (`mklink /J`), DCO commit-msg hook, statusline, sounds, memory layout user-defined. **Pending end-to-end verification:** brain isolation via NTFS ACLs — enforcement (`harden_aios.py`) added 2026-06 and not yet run on a provisioned machine, so criteria #5 (brain denied `sbin`) and #6 (audit log captures an event) are unconfirmed until `bootstrap.ps1` is run and `doctor.py` passes the ACL checks. Primary reference implementation. | [deployment/desktop.md](deployment/desktop.md) · [ReadMeToSetupYourSystem.md](getting_started/ReadMeToSetupYourSystem.md) |

...
The term "AI Operating System" is used inconsistently in the industry. Most products marketed as an "AI OS" are actually what Horizon.AIOS calls a **Brain** — an expert system finely tuned and calibrated to do one class of things well: write media, manage a business workflow, produce code, perform a specific business function.

That framing maps more cleanly to "App" than "OS."

**Horizon.AIOS vocabulary:**

| Term | Definition |
|---|---|
| **Brain** | One atomic unit of an expert system. An agentic workflow given precise training and calibration to be an expert at one specific thing. A brain is an App — it runs inside Horizon.AIOS. Every brain includes a memory system; that is what makes it an expert rather than a stateless query processor. |
| **Memory System** | The memory subsystem owned and contained within a brain — entirely the brain author's choice. Horizon.AIOS does not implement or prescribe a memory technology; it provides only the secure, isolated directory where the brain's memory lives. A brain's self-contained memory is what makes it portable: when the directory moves, the expertise moves with it, regardless of harness or model. |
| **AI Operating System (Horizon.AIOS)** | The collection of configuration, harnesses, security boundaries, logging, and coordination scaffolding that brains run on top of. The OS layer is not itself an expert system. It is the infrastructure that makes expert systems safe, auditable, and controllable. |
...
7. **How do we audit what it does?** — The audit mechanism itself: where logs are written, who controls them, whether the brain can tamper with or delete them, and how they integrate with existing SIEM and security tooling. The answer must be structural, not a promise — the brain must be physically incapable of modifying the audit trail.
...
**What Horizon.AIOS provides vs. what you must implement:**

Horizon.AIOS answers questions 2–7 fully for the Horizon.AIOS layer: provisioning records, zero-default access, explicit ACLs, credential containment, and a log directory the brain cannot write to (enforced by Deny ACE). Question 1 — *what did the agent actually do during a session* — requires OS-level audit that Horizon.AIOS cannot provide portably across all platforms and deployments. Horizon.AIOS provides the log directory, the filesystem change monitor (`horizon_aios_monitor.py`), and the isolation boundary. Full session-level audit requires integration with the infrastructure your organization already runs.
...
Horizon.AIOS gives you the structure. The integration is yours to implement — it plugs directly into whatever audit and SIEM infrastructure your organization already operates.

### The Improper Installation Risk — Running AI as Root

The security model Horizon.AIOS provides is contingent on one prerequisite: **the human operator uses a separate administrative account to manage Horizon.AIOS, and AI harnesses run under brain accounts — not the operator's account.**

When a developer runs an AI harness under their own full-control account — the default for most AI tooling installations — the harness inherits everything that account can touch. On a developer workstation where the user holds local Administrator rights (common by convention on developer machines), this is **functionally equivalent to running the AI as `root` on Linux**: the harness can read any file on the system, write to system directories, install software, access credential stores, modify network configuration, and potentially reach anything the OS allows the account to touch — including sensitive paths like `/etc/passwd`, `/etc/shadow`, `C:\Windows\System32`, and the Windows Registry.

UAC (Windows User Account Control) provides some protection, but is not a containment boundary. UAC governs elevation prompts for interactive sessions; it does not prevent a process running under an admin-group account from accessing most of the filesystem. It can be bypassed by misconfigured auto-elevation settings, COM object abuse, or elevation grants the developer has applied for development convenience. A harness that the user has granted broad tool permissions "for ease of use" may operate with effectively unrestricted access regardless of UAC state.

**Horizon.AIOS was intentionally designed to address this exact risk.** The brain OS account model exists to remove AI harnesses from the administrative trust boundary. When Horizon.AIOS is correctly deployed — with a separate administrative account and dedicated brain accounts for harness execution — the risk above does not apply. The harness's access is structurally bounded by the brain account's ACLs, not by what the operator happens to own.

**When the basic model is not followed, the user is assuming significant risk:**
- Running a harness under an administrator or full-control account means the harness can reach everything that account can reach. There is no Horizon.AIOS configuration that compensates for this — not `settings.json`, not `agents.md`, not any harness-level deny list.
- The `settings.json` deny array and harness-layer permissions in Horizon.AIOS are defense-in-depth. They are not the containment boundary. The containment boundary is the OS account. Without a separate, restricted OS account for the brain, there is no containment boundary.
- An improperly installed Horizon.AIOS — one where the harness runs as the owner account — short-circuits the entire security model the system was built to provide. The user retains all the configuration overhead of Horizon.AIOS with none of the security properties.

The Horizon.AIOS security model is not self-enforcing if its deployment prerequisites are ignored. It requires the operator to implement the basic administrative hygiene the model depends on: a dedicated administrative account for Horizon.AIOS management, and brain OS accounts for AI workloads. Without this, the system is not insecure because of a flaw — it is insecure because the fundamental premise was not applied.
...
- **User isolation** is done by the OS (NTFS ACLs / POSIX permissions), not by Horizon.AIOS code.
...
**The design cribs directly from Linux.** The POSIX model — dedicated service accounts, group memberships, `chmod` / `setfacl` filesystem permissions, process identity = OS user — is the conceptual reference for brain isolation. A brain account is a service account. Its permissions are set by an administrator at provisioning time using exactly the same tools used for any other service (`icacls` on Windows, `chmod` / `setfacl` on Linux), not by anything the AI does. On Linux the model is native. On Windows, NTFS ACLs map directly onto the same concepts with different syntax. The blue team questions in §3 — *What is the agent doing? With what access? Enforced how?* — have the same answers as they would for a web service running under a dedicated `www-data` account: the OS account, the ACLs, and the audit log tell the story.
...
3. **Docker container (Horizon.AIOS as container)** — the entire Horizon.AIOS layer runs in a container. Brains run as OS users within the container, exactly as in the native OS model. Enables consistent deployment across machines, cloud environments, and CI/CD pipelines. This is the supported Docker deployment mode.

4. **Docker container (per-brain)** — each brain runs in its own container. This is outside Horizon.AIOS scope and is a user implementation choice. BYOH applies here: if a brain's expert function requires container isolation, the brain author provisions and manages that container. The Horizon.AIOS OS layer doesn't change; the brain's working directory and Claude configuration still live under `$HORIZON_ROOT/brains/<name>/`. The containerization of the brain itself is the author's engineering decision, not a Horizon.AIOS feature.

The desktop model is the design reference: if a feature works correctly on a user's desktop machine, it will work in server and container deployments. If a feature only works in server or container mode, it is not a core Horizon.AIOS feature — it is a deployment-specific extension.
...
- Bootstrap scripts are the single source of truth for setup — cloning the repo + running bootstrap produces a working Horizon.AIOS.
- Brain provisioning is a single script invocation (`horizon_aios_create_brain.py`) — not a sequence of manual steps.
- Configuration is file-driven — the entire Horizon.AIOS state can be recreated from the repo + a local config file.

Horizon.AIOS Docker images are Linux-container based and run on Windows, macOS, and Linux hosts via the container runtime (Docker Desktop / WSL2 on Windows and macOS, Docker Engine on Linux). Native Windows containers are out of scope.
...
**Horizon.AIOS does not implement a memory system. That is entirely the brain author's decision.**

Horizon.AIOS provides one thing relevant to memory: a secure, isolated directory for each brain where anything the brain author wants to store can live — calibration documents, few-shot examples, vector indices, conversation history, retrieval scripts, domain knowledge files, or any other memory technology the author chooses. The directory has filesystem permissions that enforce isolation. What goes inside it is not Horizon.AIOS's concern.
...
- Any memory system Horizon.AIOS prescribed would either be too constrained for some brains or too complex for simpler ones.
- The "bring your own memory" model is the same as the broader BYOH principle — Horizon.AIOS provides the container and the security; the author provides the specialization.
...
- Nothing else. Memory architecture is entirely outside the Horizon.AIOS layer.
...
Any workflow or knowledge base set up in Horizon.AIOS-standard format is portable across harnesses. The same brain configuration should work whether the harness is Claude Code today or something else tomorrow. Harness-specific configuration lives in `$HORIZON_SYSTEM/harness_configs/<vendor>/` and extends — but never replaces — the cross-harness core.
...
- **Ollama** — a local model runtime with no equivalent to Claude Code's hook system or `~/.claude/` config. The open question: whether a parallel config convention (e.g., Modelfile SYSTEM blocks) can carry enough of the Horizon.AIOS context-loading and hook semantics to make brain isolation meaningful. A minimal stub exists under `harness_configs/ollama/`; it does not yet answer this question.
- **Codex** (OpenAI) — OpenAI's coding agent / CLI tooling. The open question: whether it has a config home analogous to `~/.claude/`, supports something equivalent to hooks, and can load Horizon.AIOS context in the same way `@`-imports load it for Claude Code.
...
| Value | Gap | Status |
|---|---|---|
| IaC / Containerization | ~~No Dockerfile or Docker Compose template exists.~~ | **Addressed** — `horizon_system/templates/docker/` contains Dockerfile, docker-compose.yml, and .dockerignore. `bootstrap_docker.sh` is the container-aware bootstrap. ACL hardening now runs as root before the `USER aios` switch. Per-brain containerization is outside Horizon.AIOS scope (see §5). |
| Blue Team Answerability | Horizon.AIOS logs lifecycle events (Stop/PermissionRequest/StopFailure) and Horizon.AIOS-layer file changes, but not tool invocations or reads inside brain folders. Full session-level audit requires OS-level infrastructure integration. | **Addressed by design** — Horizon.AIOS provides the boundary and the log location; §3 now documents concrete OS-level audit recommendations (Linux `auditd`, macOS BSM, Windows Security Audit, Docker logging drivers). Integration is the operator's responsibility, consistent with how every other application in their environment is audited. |
| Brain vs. Horizon.AIOS conceptual vocabulary | The distinction between Brain (expert app) and Horizon.AIOS (OS layer) was implicit. | **Addressed** — this document defines the vocabulary. |

...
Harness-agnostic agent instructions: orchestration model, session-start checklist, skills conventions, commit rules. This file is consumed by Claude Code and other harnesses (Codex, OpenHands). Do not put `@`-imports here with the intent that they will be inlined — they will not be. If you need a file inlined, `@`-import it from a `CLAUDE.md`.

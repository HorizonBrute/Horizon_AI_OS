...
|---|---|---|---|
| create-brain | `/create-brain` | `#highcap` | Provision a new brain — OS user, groups, workspace, shell profile, keystore credential (Admin/root) |
...
| objective | `/objective` | `#midcost` | Create/list/show/update durable multi-session objectives that handoffs chain back to |
...
| horizon_aios_dev_consistency_check | `/horizon_aios_dev_consistency_check` | `#highcap` | Run an iterative docs/implementation consistency validation pass against the consistency-check standard |
...
| model-prefs-assign | `/model-prefs-assign` | `#midcost` | Audit AIOS skills for their model-preference group callout and assign/refresh it, keeping both skill indexes' Model-group columns in sync |

...
| test-agent-teams | `/test-agent-teams` | `#midcost` | End-to-end self-test: walk every defined agent team, run an example NL flow, spawn each role so it echoes a nonce + its role + the model it actually ran as; reports a per-team PASS/FAIL of resolution + model routing |

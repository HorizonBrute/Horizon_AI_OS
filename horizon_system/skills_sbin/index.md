...
| post-update | `/post-update` | `#midcost` | Post-update reconciliation for a deployed install — diff what an update changed, classify inert-vs-needs-reapply, print a prioritized reapply checklist, and drive the steps by delegating to /doctor, /harden, /resync-user-skills, and the maintenance-schedule script (manual deployer tool, not scheduled) |
| context-cost-tree | `/context-cost-tree` | `#lowcost` | Map harness context overhead across the whole AIOS as ASCII-tree art — walks every directory under $HORIZON_ROOT, attributes each auto-load CLAUDE.md / agents.md / @-import file to where it lives, and reports per-directory, subtree, and grand-total tokens. Symlink → skills_bin/context-cost-tree. |

...
if [ -n "$used_pct" ] && [ -n "$session_id" ]; then
  ...
  new_threshold=0
  for t in 30 40 50 60 70 80 90; do
    if [ "$pct_int" -ge "$t" ] && [ "$last_fired" -lt "$t" ]; then
      new_threshold=$t
    fi
  done
  if [ "$new_threshold" -gt 0 ]; then
    wav="$SCRIPT_DIR/../sounds/claude_event_sounds/claude_at_${new_threshold}_statusline.wav"
    bash "$SCRIPT_DIR/../sounds/play_sound.sh" "$wav" &
